...
public class functions_by_genre {
	public boolean belong(double n, double n1, double n2){
		if (n > n1 && n <= n2){
            return true;
        }
        else return false;
	...
	public boolean belong2(double n, double n1, double n2){
        if (n >= n1 && n < n2){
            return true;
        }
        else return false;
	...
	public List<String> getSame(Big5Entity big, double value){
	...
	public List<String> getGenre(String ID){
		List<String> generi= new ArrayList<String>();
		DBfunctions fun=new DBfunctions();
		Big5Entity big=fun.getPersonality(ID);
		String gender=fun.getQuestionary(ID).getSex();
		double ope=big.getApe();
		double con=big.getCosc();
		double ext=big.getEstr();
		double agr=big.getGrad();
		double neu=big.getNevr();
		...
		if(generi.size()==0){
            ...
            if (min-1 < 5-max)
                traits=getSame(big,min); //chiavi con il valore min
            else if (min-1 > 5-max)
                traits = getSame(big, max); //chiavi con il valore max
            else
                traits = getSame(big, min); //chiavi con il valore max e min
