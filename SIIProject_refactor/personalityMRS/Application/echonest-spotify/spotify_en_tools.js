...
function getSpotifyPlayer(inPlaylist, callback) {
    ...
    function addSpotifyInfoToPlaylist() {
        var cont=0;
        //var lengthArr = inPlaylist.length-1;
        for(var x in inPlaylist ){
            (function(x){
             
            var tids = [];
            inPlaylist[x].forEach(function(song) {
                           var tid = fidToSpid(song.tracks[0].foreign_id);
                           tids.push(tid);
                           });
            ...
            .done(function(data) {
                  data.tracks.forEach(function(track, i) {
                                  inPlaylist[x][i].spotifyTrackInfo = track;
                                  });
              
                  var playlistTemp = filterSongs(inPlaylist[x]);
                  if(cont==10){
                    callback(player);
                  }else if(playlistTemp.length != 0){
                    playlist.push(playlistTemp[0]);
                    showCurSong(false);
                    cont++;
                    if (x==inPlaylist.length-1) {
                        callback(player);
                    }
                  }
            })
